...
class MenuCtrl {
    ...
    async getAll(req: Request, res: Response) {
        try {
            const menus = await MenuModel.find().populate('_shop').populate('_categori')
            res.json(menus)
        } catch (error) {
    ...
    async get(req: Request, res: Response) {
        try {
            const menu = MenuModel.findById(req.params.id).populate('_shop').populate('_categori')
            res.json(menu)
        } catch (error) {
            res.status(500).json(error)
        }
    ...
    async add(req: Request, res: Response) {
        try {
            ...
            const newMenu = await menu.populate('_shop').populate('_categori').execPopulate()
